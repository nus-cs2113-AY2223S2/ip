...
public class StorageFile {
    ...
    public static final String ERROR_UNKNOWN_TASK = "There is no way you can see this\n";
    //Nakiri Ayame
    ...
    public static void initialiseData(TaskList taskList) {
        ...
        try {
            if (data.createNewFile()) {
                ...
                readPreviousData(taskList,in,data);
    ...
    /**
     * Helper function to read the data.txt specified in DATA_PATH and update the taskList during initialisation
     * Will immediately quit and destroy data.txt if file is corrupted.
     *
     * @param taskList the empty TaskList
     * @param in the Scanner used to read data.txt
     * @param data a File linked to data.txt
     */
    private static void readPreviousData(TaskList taskList, Scanner in, File data) {
        while (in.hasNextLine()) {
            String[] inputMessage = parser.Parser.processInputMessage(in);
            boolean fileIsBroken = processDataFileByLine(inputMessage, taskList);
            if (fileIsBroken) {
     ...
     */
    private static boolean processDataFileByLine(String[] inputMessage, TaskList taskList) {
        ...
        try {
            ...
            for (int i = 0; i < sizeOfTaskList; i += 1) {
                ...
                case ACTION_NEW_EVENT:
                    ...
                    break;
                default:
                    instruction = ERROR_UNKNOWN_TASK;
                }
